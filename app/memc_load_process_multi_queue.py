...
def insert_appsinstalled(queue_in, queues_out, device_memc, processed, errors, dry_run=False):
    i = 0
    while True:
        ...
        queue_out = queues_out[appsinstalled.dev_type]
...
def load_to_memcache(queue_in, memc, errors):
    i = 0
    while True:
        task = queue_in.get()
...
def main(options):
    ...
    for fn in glob.iglob(options.pattern):
        ...
        load_qs = {}
        load_ps = {}
        for dev, addr in device_memc.items():
            q = Queue()
            p = Process(target=load_to_memcache, args=(q, get_memcache(addr), errors))
            # p.start()
            load_ps[dev] = p
            load_qs[dev] = q
        ...
        for i in range(options.pack_p):
            p = Process(target=insert_appsinstalled, args=(pack_q, load_qs, device_memc, processed, errors, options.dry), name=f"Pack {i}")
            p.start()
            pack_ps.append(p)

        for p in load_ps.values():
            p.start()
