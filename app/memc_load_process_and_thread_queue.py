...
class read_log_worker(threading.Thread):
    def __init__(self,fn, queue_out):
    ...
    def run(self) :
        ...
        for line in fd:
            i += 1
            if i % 100000 == 0:
                logging.info(f"{self.name} - Read {i} from {self.fn}")
            if MAX_LINES and (i % MAX_LINES == 0):
                logging.info(f"{self.name} - Read from {self.fn} end, {i} processed")
                fd.close()
                break
            line = line.strip()
            ...
            if not line:
                continue
            self.queue_out.put(line)


class load_to_memcache_worker(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            if isinstance(task, str) and task == SENTINEL:
                logging.info(f"{self.name} - Load end, {i} processed")
            ...
            i += 1
            if i % 100000 == 0:
                logging.info(f"{self.name} - Load {i}")
...
def main(options):
    ...
    for fn in glob.iglob(options.pattern):
        ...
        for i in range(READ_PROCESSES):
            p = read_log_worker(fn, parse_q)
            p.start()
            read_ps.append(p)

        parse_ps = []
        for i in range(PARSE_PROCESSES):
            p = Process(target=parse_appsinstalled, args=(parse_q, pack_q, errors), name=f"Parse {i}")
            p.start()
            parse_ps.append(p)

        pack_ps = []
        for i in range(PACK_PROCESSES):
            p = Process(target=insert_appsinstalled, args=(pack_q, load_q, device_memc, processed, errors, options.dry), name=f"Pack {i}")
            p.start()
            pack_ps.append(p)

        load_ps = []
        for i in range(LOAD_PROCESSES):
            p = load_to_memcache_worker(load_q)
            p.start()
            load_ps.append(p)

        for p in read_ps:
            p.join()
            logging.info(f"read thread {p.name} end")
        ...
        for p in load_ps:
            p.join()
            logging.info(f"load process {p.name} end")
